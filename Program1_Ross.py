...
for i in range(0, triangle+1): #counts from 0 to the range defined by the user input
    for j in range(triangle- i): #counts to the user input minus the stage of the previous for loop
        print(" ", end = " ") #prints until the value of the level of the pyramid minus 1
    for j in range(1, i): #puts the two halfs of the pyramid together
        print(building_block, end = " ") #the output displays the user's choice of building block and breaks to a new line  
    for i in range(i,0,-1): #makes the right half of the pyramid
